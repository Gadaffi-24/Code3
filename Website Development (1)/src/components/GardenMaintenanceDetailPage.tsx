import DescriptionGardenMaintenance from '../imports/DescriptionGardenMaintenance';

interface GardenMaintenanceDetailPageProps {
  onNavigate?: (page: string) => void;
}

export default function GardenMaintenanceDetailPage({ onNavigate }: GardenMaintenanceDetailPageProps) {
    ...
    <div 
      onClick={(e) => {
        ...
        if (target.textContent === 'Home' || target.closest('p')?.textContent === 'Home') {
          ...
          e.preventDefault();
          alert('Course added! Redirecting to courses page...');
          onNavigate?.('courses');
        }
      }}
      className="cursor-pointer"
    >
      <DescriptionGardenMaintenance />
    </div>
  );
}
